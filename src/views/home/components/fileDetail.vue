  ...
  <div class="main">
    ...
    >
      ...
      <a-upload-dragger
        name="file"
        :multiple="true"
        :action="baseUrl + '/file/uploadFile2'"
        @change="handleChange"
        :headers="headers"
        :data="fileInfo"
      >
    ...
    <!-- 项目信息 -->
    <a-descriptions title="文件信息" :bordered="true">
      <a-descriptions-item label="文件夹名称">
        {{ fileInfo["file_name"] }}
      </a-descriptions-item>
      <a-descriptions-item label="创建时间">
        {{ moment(+fileInfo["date"]).format("YYYY-MM-DD") }}
      </a-descriptions-item>
    </a-descriptions>
...
export default {
  ...
  methods: {
    async getTableData() {
      ...
      const getFileTableData = await this.$WebApi["home"].selectFileTreeByID({
    ...
    async onAddFolder() {
      ...
      //上传fileFolder
      let projectId = this.fileInfo.project_id;
      let pid = this.fileInfo.id;
      let fileName = this.form.name;
      let saveFile = this.fileInfo["save_file"];
